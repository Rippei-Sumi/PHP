...
if ($user) {
    ...
    // 有効期限：30分
    $stmt = $pdo->prepare(
        'INSERT INTO password_resets (user_id, token_hash, expired_at)
         VALUES (:user_id, :token_hash, NOW() + INTERVAL 30 MINUTE)'
    );
    $stmt->execute([
        ':user_id' => $user_id,
...
/* 常に同じ画面へ（存在チェック対策） */
header('Location:user_reset_password_mailed.php');
exit;
